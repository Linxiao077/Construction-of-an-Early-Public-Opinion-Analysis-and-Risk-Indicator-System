"""
基于第二轮伪标签 PL2' 刷新烟雾词表，得到 v2：

流程: PL2' → v2

本脚本是 risk_smokewords_v1.py 的 M2/PL2' 版本：
    - 使用 data_combined_4types.xlsx + pseudo_labels_pl2_M2.csv
    - 将人工 risk 与 PL2' 合并为 final_risk
    - 在 0–3 档标签下重新计算:
        df_r(t)、df_defect(t)、df_nondefect(t)、rtw(t)、risk_avg(t)
    - 按阈值筛选生成烟雾词表 v2

输出:
    - riskword_term_stats_v2.csv
    - smokewords_v2.csv
"""
...
from smokewords_stats import tokenize  # 与 v0/v1 保持分词口径一致
...
def load_data_and_pl2() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not os.path.isfile(PL2_PATH):
        raise FileNotFoundError(f"未找到 PL2' 文件: {PL2_PATH}，请先运行 risk_bert_riskmodel_M2.py。")
...
def attach_final_labels(df: pd.DataFrame, pl2: pd.DataFrame) -> pd.DataFrame:
    """
    将人工 risk 与 PL2' 合并成最终标签列 'final_risk':
    ...
    print(
        f"[INFO] 基于人工标签 + PL2'，可用于统计的样本数: {len(labeled_df)} "
...
def build_smokewords_v2(stats_df: pd.DataFrame) -> pd.DataFrame:
    cond_support = stats_df["df_defect"] >= MIN_DF_DEFECT
    cond_rtw = stats_df["rtw"] >= RTW_THRESHOLD
    cond_avg = stats_df["risk_avg"] >= RISK_AVG_MIN

    v2 = stats_df[cond_support & cond_rtw & cond_avg].copy()
    v2["is_high_risk"] = v2["risk_avg"] >= RISK_AVG_HIGH
    v2 = v2.sort_values(["rtw", "risk_avg"], ascending=False).reset_index(drop=True)
    return v2
...
def main():
    ...
    stats_path = "riskword_term_stats_v2.csv"
    stats_df.to_csv(stats_path, index=False, encoding="utf-8-sig")
    print(f"[INFO] 已保存 v2 词统计到: {stats_path} (共 {len(stats_df)} 个 term)")

    v2_df = build_smokewords_v2(stats_df)
    v2_path = "smokewords_v2.csv"
    v2_df.to_csv(v2_path, index=False, encoding="utf-8-sig")
    print(f"[INFO] 已保存烟雾词词表 v2 到: {v2_path} (共 {len(v2_df)} 个词)")

    print("\n[INFO] v2 高风险烟雾词示例前 20 行：")
    print(v2_df.sort_values("risk_avg", ascending=False).head(20).to_string(index=False))
